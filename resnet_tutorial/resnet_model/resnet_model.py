...
class BasicBlock(nn.Module) :
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None) :
        ...
        if groups != 1 or base_width != 64 :
            # In the ResNet architecture, the base width refers to the number of filters in the convolutional layers of the network.
            raise ValueError('BasicBlock only supports groups=1 and base_width=64')
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in BasicBlock") # kernel ���� ����
...
class Bottleneck(nn.Module) :
    expansion = 4
    # Bottleneck(inplanes=64, planes=3, stride=1, downsample=None, groups=1, base_width=64, previous_dilation=1, norm_layer=None)
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None) :
        ...
        width = int(planes * (base_width / 64.))*groups #width=3, 64
...
class ResNet(nn.Module) :
    # layers = [3, 4, 6, 3]
    def __init__(self, block, layers, num_classes=17, zero_init_residual=False, groups=1, 
...
# def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None)
        # Bottleneck(inplanes=64, planes=64, stride=1, downsample=None, groups=1, base_width=64, previous_dilation=1, norm_layer=None)
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, norm_layer))

        self.inplanes = planes * block.expansion #256, 64
        # 동일 블록 반복 
